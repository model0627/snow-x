<script lang="ts">
	...
	async function addApiConnection() {
		if (newConnection.name && newConnection.url) {
			try {
				const headers = newConnection.headers.reduce(
					(acc, header) => {
	...
	function addFieldMapping() {
		newConnection.fieldMapping.mappings = [
			...newConnection.fieldMapping.mappings,
			{
				source_field: '',
				target_field: '',
	...
	function addFilterCondition() {
		newConnection.fieldMapping.filter_conditions = [
			...newConnection.fieldMapping.filter_conditions,
			{
				field: '',
	...
	function removeFilterCondition(index: number) {
		newConnection.fieldMapping.filter_conditions = newConnection.fieldMapping.filter_conditions.filter(
			(_, i) => i !== index
		);
	}
...
<div class="dark:bg-mofu-dark-800 bg-mofu-light-800 rounded-lg border p-6">
	<div class="mb-6 flex items-center justify-between">
		...
		<Button onclick={() => (showAddForm = !showAddForm)} class="bg-orange-500 text-white hover:bg-orange-600">
			+ API 연결 추가
		</Button>
	</div>
	...
	<!-- Add New API Form -->
	{#if showAddForm}
		<div class="dark:bg-mofu-dark-700 bg-mofu-light-700 mb-6 rounded-lg border p-6">
			<h3 class="mb-6 text-base font-semibold">새 API 연결 추가</h3>

			<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
				<!-- 좌측 열 -->
				<div class="space-y-4">
					<div>
						<label for="api-name" class="mb-2 block text-sm font-medium">
							연결 이름 <span class="text-red-500">*</span>
						</label>
						<Input id="api-name" bind:value={newConnection.name} placeholder="예: Kandji MDM API" class="text-sm" />
					</div>

					<div>
						<label for="api-category" class="mb-2 block text-sm font-medium">
				...
				<div class="space-y-4">
					<div>
						<label for="api-type" class="mb-2 block text-sm font-medium">
			...
			<!-- API URL -->
			<div class="mt-6">
				<label for="api-url" class="mb-2 block text-sm font-medium">
				...
				<div class="space-y-2">
					<div class="flex gap-2">
						...
						<Button
							...
							variant="default"
							class="bg-blue-500 px-4 text-white hover:bg-blue-600"
					...
					{#if testConnectionResult}
						<div
							class="rounded-md p-3 text-sm {testConnectionResult.status === 'success'
								? 'border border-green-200 bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400'
								: 'border border-red-200 bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400'}"
						>
			...
			<!-- 설명 -->
			<div class="mt-6">
				<label for="api-description" class="mb-2 block text-sm font-medium">설명</label>
				<Textarea
					id="api-description"
			...
			<div class="mt-6">
				...
				{#each newConnection.headers as header, index}
					<div class="mb-2 flex gap-2">
						<Input bind:value={header.key} placeholder="헤더 이름 (예: Authorization)" class="flex-1 text-sm" />
						<Input bind:value={header.value} placeholder="헤더 값 (예: Bearer token123)" class="flex-1 text-sm" />
			...
			<div class="mt-6">
				<div class="flex items-center gap-3">
					<input type="checkbox" id="auto-sync" bind:checked={newConnection.autoSync} class="h-4 w-4" />
			...
			<div class="mt-6">
				<div class="mb-3 flex items-center justify-between">
					...
					<Button onclick={() => (showFieldMapping = !showFieldMapping)} variant="ghost" size="sm" class="text-sm">
						{showFieldMapping ? '숨기기' : '고급 설정'}
					</Button>
				</div>

				{#if showFieldMapping}
					<div class="dark:bg-mofu-dark-600 bg-mofu-light-600 space-y-4 rounded-lg border p-4">
						<!-- 필드 매핑 -->
						<div>
							<div class="mb-3 flex items-center justify-between">
								<h4 class="text-sm font-medium">데이터 필드 매핑</h4>
								<div class="flex gap-2">
									{#if sampleDataForMapping}
										<Button onclick={openFieldMappingDialog} variant="outline" size="sm" class="text-sm">
							...
							{#each newConnection.fieldMapping.mappings as mapping, index}
								<div class="mb-3 grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-5">
									<div>
										<Input bind:value={mapping.source_field} placeholder="원본 필드명" class="text-xs" />
									</div>
									<div class="flex items-center justify-center">
										<Icon src={ArrowRight} size="16" class="text-gray-400" />
									</div>
									<div>
										<Input bind:value={mapping.target_field} placeholder="대상 필드명" class="text-xs" />
									</div>
									<div>
									...
									<div class="flex gap-1">
										<Input bind:value={mapping.default_value} placeholder="기본값" class="flex-1 text-xs" />
										{#if newConnection.fieldMapping.mappings.length > 1}
											<Button onclick={() => removeFieldMapping(index)} variant="ghost" size="sm" class="text-red-500">
						...
						<div>
							<div class="mb-3 flex items-center justify-between">
								<h4 class="text-sm font-medium">데이터 필터 조건</h4>
							...
							{#each newConnection.fieldMapping.filter_conditions as condition, index}
								<div class="mb-2 grid grid-cols-1 gap-2 rounded-md border p-2 md:grid-cols-4">
									<div>
										<Input bind:value={condition.field} placeholder="필드명" class="text-xs" />
									</div>
									<div>
									...
									<div>
										<Input bind:value={condition.value} placeholder="값" class="text-xs" />
									</div>
									<div>
										<Button onclick={() => removeFilterCondition(index)} variant="ghost" size="sm" class="text-red-500">
											<Icon src={Trash} size="14" />
										</Button>
									</div>
								</div>
							...
							{#if newConnection.fieldMapping.filter_conditions.length === 0}
								<div class="py-2 text-center text-xs text-gray-500">필터 조건이 없으면 모든 데이터가 동기화됩니다.</div>
							{/if}
						</div>
					</div>
				{/if}
			...
			<div class="mt-8 flex justify-end gap-3 border-t pt-4">
				...
				<Button onclick={addApiConnection} class="bg-orange-500 text-white hover:bg-orange-600">API 연결 추가</Button>
			</div>
		</div>
	{/if}
	...
	<div class="space-y-4">
		{#if isLoading}
			<div class="py-8 text-center">
				<div class="mx-auto h-8 w-8 animate-spin rounded-full border-b-2 border-orange-500"></div>
				<p class="mt-2 text-sm text-gray-500">API 연결 목록을 불러오는 중...</p>
			</div>
		{:else}
			{#each apiConnections as connection}
				<div class="dark:bg-mofu-dark-700 bg-mofu-light-700 rounded-lg border p-4">
					<div class="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
						<!-- 기본 정보 -->
						<div class="min-w-0 flex-1">
							<div class="mb-2 flex items-center gap-3">
								<h3 class="text-base font-medium">{connection.name}</h3>
								<span
								...
								>
									{connection.status === 'active' ? (connection.hasError ? '에러' : '성공') : '비활성'}
								</span>
							</div>

							{#if connection.hasError && connection.errorMessage}
								<div
									class="mb-2 rounded border border-red-200 bg-red-50 p-2 text-xs text-red-700 dark:bg-red-900/20 dark:text-red-400"
								>
									<strong>에러:</strong>
							...
							<div class="mb-2 font-mono text-xs break-all text-gray-500">
								{connection.url}
							</div>
							...
							<div class="grid grid-cols-1 gap-2 text-xs sm:grid-cols-2 lg:grid-cols-4">
								...
								<div class="flex items-center gap-2">
									...
									<Switch checked={connection.syncEnabled} onCheckedChange={() => toggleSync(connection.id)} />
								</div>
							</div>
						</div>
						...
						<div class="flex flex-shrink-0 items-center gap-2">
							...
							<Button variant="ghost" size="sm" onclick={() => startEditConnection(connection)} aria-label="수정">
								<Icon src={Pencil} size="16" />
							</Button>

							<Button variant="ghost" size="sm" onclick={() => viewConnection(connection)} aria-label="보기">
			...
			{#if apiConnections.length === 0}
				<div class="py-12 text-center">
					<Icon src={Link} size="40" class="mx-auto mb-4 text-gray-400" />
					<p class="mb-2 text-sm text-gray-500">연결된 외부 API가 없습니다</p>
