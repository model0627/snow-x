<script lang="ts">
	...
	async function loadData() {
		...
		try {
			...
			if (searchQuery.trim()) {
				const filtered = allServerRooms.filter(
					(room) =>
		...
		<div class="relative max-w-md">
			...
			<input
				...
				class="w-full py-2 pr-4 pl-10 {isDesktop ? 'text-xs' : 'text-sm'} rounded-lg border border-gray-300 bg-gray-50
					text-gray-900 focus:ring-2 focus:ring-blue-500 focus:outline-none
								...
								<div class="flex items-start gap-3">
									...
									<div class="flex-1">
										...
										{#if serverRoom.description}
											<p class="{isDesktop ? 'text-[10px]' : 'text-xs'} mt-1 text-gray-500 dark:text-gray-400">
							...
							<div class="flex items-center justify-between">
								...
								<div class="flex items-center gap-2">
									...
									<button
										...
										class="{isDesktop
											? 'text-xs'
											: 'text-sm'} flex items-center gap-1 text-gray-600 transition-colors hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"
