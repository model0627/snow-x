use crate::api::v0::routes::device::handlers::{
    assign_ip_to_device, create_device, delete_device, get_device_by_id,
    get_device_ip_addresses, get_devices, unassign_ip_from_device, update_device,
};
...
pub fn create_device_routes() -> Router<crate::AppState> {
    Router::new()
        ...
        .route(
            "/{id}/ip-addresses",
            get(get_device_ip_addresses)
                .route_layer(axum::middleware::from_fn(access_jwt_auth)),
        )
        .route(
            "/{id}/ip-address",
            post(assign_ip_to_device)
                .route_layer(axum::middleware::from_fn(access_jwt_auth)),
        )
        .route(
            "/{id}/ip-address/{ip_address_id}",
            delete(unassign_ip_from_device)
                .route_layer(axum::middleware::from_fn(access_jwt_auth)),
        )
}
