<script lang="ts">
	...
	async function handleSubmit() {
		...
		try {
			...
			// };
			// await privateApi.post('v0/ipam/ip-ranges', { json: createData });
...
<Dialog {open} onOpenChange={(value) => !value && onClose()}>
	<DialogContent class="max-w-2xl w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-xl">
		<DialogHeader class="pb-4 border-b border-gray-200 dark:border-gray-700">
			<div class="flex items-center gap-3">
				...
				<DialogTitle class="{isDesktop ? 'text-lg' : 'text-xl'} text-gray-900 dark:text-white font-semibold">
					새 IP 대역 추가
				</DialogTitle>
			</div>
		</DialogHeader>
		...
		<form on:submit|preventDefault={handleSubmit} class="pt-4 space-y-5">
			<!-- IP 대역명 -->
			<div>
				<label for="name" class="block {isDesktop ? 'text-sm' : 'text-base'} font-medium text-gray-900 dark:text-white mb-2">
				...
				<input
					...
					required
					class="w-full px-3 py-2.5 {isDesktop ? 'text-sm' : 'text-base'} border border-gray-300 dark:border-gray-600 rounded-lg
			...
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
				<div class="md:col-span-2">
					<label for="network_address" class="block {isDesktop ? 'text-sm' : 'text-base'} font-medium text-gray-900 dark:text-white mb-2">
				...
				<div>
					<label for="cidr_mask" class="block {isDesktop ? 'text-sm' : 'text-base'} font-medium text-gray-900 dark:text-white mb-2">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div>
					<label for="gateway" class="block {isDesktop ? 'text-sm' : 'text-base'} font-medium text-gray-900 dark:text-white mb-2">
						게이트웨이
					</label>
					<input
						id="gateway"
				...
				<div>
					<label for="vlan_id" class="block {isDesktop ? 'text-sm' : 'text-base'} font-medium text-gray-900 dark:text-white mb-2">
						VLAN ID
					</label>
					<input
						id="vlan_id"
			...
			<!-- DNS 서버 -->
			<div>
				<label for="dns_servers" class="block {isDesktop ? 'text-sm' : 'text-base'} font-medium text-gray-900 dark:text-white mb-2">
					DNS 서버
				</label>
				<input
					id="dns_servers"
			...
			<!-- 설명 -->
			<div>
				<label for="description" class="block {isDesktop ? 'text-sm' : 'text-base'} font-medium text-gray-900 dark:text-white mb-2">
					설명
				</label>
				<textarea
					id="description"
			...
			<div class="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
				<Button
					...
					onclick={onClose}
					disabled={loading}
					class="{isDesktop ? 'text-sm px-4 py-2' : 'px-6 py-2'} border-gray-300 dark:border-gray-600"
				>
					취소
				</Button>
				<Button
					...
					class="bg-blue-600 hover:bg-blue-700 text-white {isDesktop ? 'text-sm px-4 py-2' : 'px-6 py-2'} disabled:opacity-50"
				>
					{loading ? '추가 중...' : '추가'}
				</Button>
			</div>
